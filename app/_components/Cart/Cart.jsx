...
function Cart() {
  ...
  return (
    openCart && (
      <div className="absolute lg:right-0 z-10 overflow-y-auto overflow-x-hidden">
        <div
          className="relative w-screen max-w-sm bg-[#1e2121e6] max-h-[500px] overflow-y-auto mr-0 lg:mr-10 shadow-lg rounded px-4 py-8 sm:px-6 lg:px-8"
          aria-modal="true"
          role="dialog"
          tabIndex="-1"
        >
              ...
              <ul className="space-y-4">
                {cart?.map((item) => (
                  <Link href={`/service_details/${item?.serviceDetail?.id}`}>
                    <li
                      key={item?.id}
                      className="flex items-center mb-1 gap-4 border rounded p-3 cursor-pointer bg-[#1e2121]"
                    >
